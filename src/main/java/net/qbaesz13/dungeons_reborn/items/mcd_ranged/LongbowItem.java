...
public class LongbowItem extends SC_BowItem {
    ...
    protected void shootProjectile(@NonNull LivingEntity shooter, Projectile projectileEntity, int index, float power, float uncertainty, float angle, @Nullable LivingEntity targetOverrride) {
        projectileEntity.shootFromRotation(shooter, shooter.getXRot(), shooter.getYRot() + angle, 0.0F, power * 1.3F, uncertainty);
        ((AbstractArrow)projectileEntity).setBaseDamage(((AbstractArrowAccessors)projectileEntity).getBaseDamage() * 1.3F);
    }
    @Override @NullMarked
    public void appendHoverText(ItemStack itemStack, TooltipContext context, TooltipDisplay display, Consumer<Component> builder, TooltipFlag tooltipFlag) {
        DungeonsHelpers.Tooltip.appendDungeonsHeader(builder);
        DungeonsHelpers.Tooltip.appendDescription(builder, Component.translatable("tooltip.dungeons_reborn.longbow"));
